...
# data_path
annotation_path = os.path.join(project_path, 'data/total_inter_10_11.csv')
assert annotation_path
metadata_path = os.path.join(project_path, 'data/meta.json')
assert metadata_path
result_path = os.path.join('/Users/qingwuliu/Desktop/results')
...
for item in tqdm(total_list):
    ...
    for lidar_point in lidar_points:
        ...
        if len(uv_list) == 8:
            ...
            if min_u < 0 or min_v < crop_height or max_u > 1632 or max_v > 1216:
                ...
                new_row_as_dict['top_2d'] = min_v - crop_height
                new_row_as_dict['right_2d'] = max_u
                new_row_as_dict['bottom_2d'] = max_v - crop_height
                ...
                #             new_row_as_dict['l_3d'], new_row_as_dict['w_3d'], new_row_as_dict['h_3d'], new_row_as_dict['heading_3d'], 
                #             new_row_as_dict['x_3d_cam'], new_row_as_dict['y_3d_cam'], new_row_as_dict['z_3d_cam']]
                if int(new_row_as_dict['class_id']) == 1:
                    class_tmp = 'Pedestrian'
                elif int(new_row_as_dict['class_id']) == 2:
                    class_tmp = 'Car'
                elif int(new_row_as_dict['class_id']) == 3:
                    class_tmp = 'Cyclist'
                elif int(new_row_as_dict['class_id']) == 4:
                    class_tmp = 'MotoCycle'
                elif int(new_row_as_dict['class_id']) == 5:
                    class_tmp = 'Bus'
                elif int(new_row_as_dict['class_id']) == 6:
                    class_tmp = 'Truck'      
                else:
                    assert 'Type not in the categories'                          

                '''.DS_Store
                # Real one: {'1': 'Pedestrian', '2': 'Car', '3': 'Cyclist', '4': 'Motocycle', '5': 'Bus', '6': 'Truck'}
                self.clsnum2type = {'0': 'Pedestrian', '1': 'Car', '2': 'Cyclist', '3': 'Cyclist', '4': 'Car', '5': 'Car', '6': 'Cyclist'}
                0. type: 'Car'
                1. truncated: 0
                2. occluded: 0
                3. alpha: 0 # not sure about this one
                4. bbox[0]
                5. bbox[1]
                6. bbox[2]
                7. bbox[3]
                8. dimension[0] # kitti: height   Lumpi: height
                9. dimension[1] # kitti: width    Lumpi: witdh
                10. dimension[2] # kitti: length  Lumpi: length 
                11. location[0] # kitti: x   Lumpi: x
                12. location[1] # kitti: y   Lumpi: z
                13. location[2] # kitti: z   Lumpi: y
                14. rotation_y 
                15. depth
                16. score
                '''  
                new_list_1 = [class_tmp, 0, 0, 0, 
                              new_row_as_dict['left_2d'], new_row_as_dict['top_2d'], 
                              new_row_as_dict['right_2d'],  new_row_as_dict['bottom_2d'],
                              new_row_as_dict['h_3d'], new_row_as_dict['w_3d'], new_row_as_dict['l_3d'],
                              new_row_as_dict['x_3d'], new_row_as_dict['z_3d'], new_row_as_dict['y_3d'],
                              new_row_as_dict['heading_3d'], new_row_as_dict['z_3d_cam']]             
                # print(f'new_list: {new_list}')
                total_frame_dict[str(int(current_row_as_dict['frame_id']))].append(new_list_1)
